...
export function PatientDetail({ patientId, onBack }: Props) {
  ...
  <div className="space-y-6">
    ...
    {/* ✅ Latest Medical Record (no view button) */}
    {medicalRecords.length > 0 ? (
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm text-gray-500 text-center">
                  ...
                  <div className="flex items-start justify-between mb-4">
                    ...
                    {record.file_url && (
                      <button
                        onClick={() => {
                          try {
                            const urls = Array.isArray(record.file_url)
                              ? record.file_url
                              : JSON.parse(record.file_url || '[]');
                            if (urls.length === 0) {
                              alert('No document found for this record.');
                              return;
                            }
                        ...
                        className="flex items-center space-x-1 px-3 py-2 bg-yellow-500 text-white rounded-md hover:bg-yellow-600 transition-all shadow-sm"
                      >
      ...
      {/* ✅ Recovery Progress */}
      {activeTab === 'recovery' && <RecoveryProgress patientId={patientId} />}
