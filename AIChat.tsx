...
export function AIChat({ onClose, selectedPatientId }: Props) {
  ...
  const surgeryInfo: Record<string, { procedure: string; process: string; learning: string; purpose?: string; risks?: string; precautions?: string; recovery?: string }> = {
    ...
    ,
    // Added procedure-specific entries requested by user
    'coronary angioplasty': {
      purpose: 'Restore coronary blood flow by dilating stenotic segments and deploying a stent when indicated to maintain vessel patency.',
      procedure: 'Percutaneous coronary intervention (PCI) with balloon angioplasty and stent deployment (commonly drug-eluting stents).',
      process: '1. Vascular access (radial or femoral)\n2. Guidewire and catheter navigation to the coronary ostium\n3. Coronary angiography to identify lesion(s)\n4. Balloon angioplasty to dilate the stenosis\n5. Deployment of a stent (drug-eluting when indicated)\n6. Post-dilatation and final angiographic assessment\n7. Hemostasis at access site and post-procedure monitoring',
      risks: 'Vessel dissection, perforation, acute stent thrombosis, restenosis, bleeding at access site, contrast nephropathy, arrhythmia.',
      precautions: 'Check renal function and allergies to contrast, dual antiplatelet therapy planning (DAPT), anticoagulation management during procedure, prepared for emergency conversion to CABG if required.',
      recovery: 'Short observation in recovery/CCU; mobilize early (particularly with radial access), DAPT typically for 6–12 months depending on stent type and clinical context, outpatient follow-up with cardiology.',
      learning: '• Indications for PCI vs CABG\n• Vascular access techniques (radial vs femoral)\n• Importance of antiplatelet therapy and adherence\n• Recognizing and managing acute complications (dissection, perforation, stent thrombosis)'
    },
    'right hemiglossectomy': {
      purpose: 'Oncologic resection of the right hemiglossal tongue to achieve clear margins for malignant lesions.',
      procedure: 'Excision of the right half of the tongue (mucosa, intrinsic and necessary extrinsic muscle) often combined with neck dissection when nodal risk is present.',
      process: '1. Intraoral assessment and margin marking\n2. Mucosal incision and stepwise resection to appropriate depth\n3. Hemostasis (control of lingual vessels)\n4. Frozen section margin assessment when available\n5. Consider immediate reconstruction (local flap or free flap)\n6. Closure and neck drain placement if neck dissection performed',
      risks: 'Bleeding, airway compromise, speech and swallowing impairment, wound dehiscence, flap failure (if reconstruction), nerve injury.',
      precautions: 'Plan airway strategy (consider tracheostomy for extensive resections), coordinate with reconstructive team if flap likely, ensure blood products available and have hemostatic measures ready.',
      recovery: 'NPO initially with enteral feeding as needed, early speech and swallow therapy involvement, flap monitoring if applicable, progressive diet advancement based on swallow assessment.',
      learning: '• Tongue anatomy and implications for speech/swallow\n• Margin assessment and indication for neck dissection\n• Principles of reconstruction to preserve function\n• Early recognition and management of postoperative airway and bleeding complications'
    },
    'neck dissection': {
      purpose: 'Remove regional lymphatic tissue to treat or stage cervical nodal metastases from head and neck cancers.',
      procedure: 'Selective, modified, or radical neck dissection depending on disease burden, with preservation of non-lymphatic structures when oncologically safe.',
      process: '1. Incision along neck crease and flap elevation\n2. Identify and preserve cranial nerves and vascular structures when possible\n3. Systematic removal of targeted levels (I–V as indicated)\n4. Hemostasis, placement of drains, and layered closure',
      risks: 'Nerve injury (spinal accessory, hypoglossal, marginal mandibular), shoulder dysfunction, chyle leak, hematoma, infection.',
      precautions: 'Preoperative imaging to plan extent, intraoperative nerve identification, readiness to manage chyle leak and vascular injury.',
      recovery: 'Drain management, early physiotherapy for shoulder mobility, pain control and wound care, outpatient follow-up for pathology and adjuvant therapy planning.',
      learning: '• Cervical lymph node levels and mapping\n• Techniques to preserve function while achieving oncologic control\n• Post-op rehabilitation to reduce shoulder morbidity'
    },
    'lumpectomy': {
      purpose: 'Breast-conserving surgery to remove a breast cancer or suspicious lesion while preserving breast appearance.',
      procedure: 'Excision of the lesion with a rim of normal tissue (wide local excision), often combined with sentinel lymph node biopsy.',
      process: '1. Pre-op imaging and lesion localization (wire or seed localization)\n2. Incision and excision with margin assessment\n3. Sentinel node identification and biopsy if indicated\n4. Specimen orientation and hemostasis\n5. Wound closure and drain if required',
      risks: 'Positive margins needing re-excision, hematoma, infection, seroma, cosmetic deformity.',
      precautions: 'Coordinate with radiology for localization, discuss oncoplastic options for better cosmesis, and plan for adjuvant radiotherapy as indicated.',
      recovery: 'Usually outpatient or short stay; wound care and activity modification for 1–2 weeks; follow-up for pathology and adjuvant treatment planning.',
      learning: '• Indications for breast conservation vs mastectomy\n• Margin assessment and re-excision thresholds\n• Role of sentinel node biopsy and adjuvant radiotherapy'
    },
    'modified radical mastectomy': {
      purpose: 'Remove the entire breast and axillary lymph nodes while preserving some non-lymphatic structures (e.g., pectoralis muscles) for oncologic control in invasive breast cancer.',
      procedure: 'Total mastectomy with level I–II (and sometimes III) axillary dissection; skin/nipple-sparing decisions depend on oncologic safety.',
      process: '1. Pre-op marking and imaging review\n2. Incision and mastectomy with careful flap dissection\n3. Axillary dissection as indicated\n4. Hemostasis, drain placement, and closure\n5. Consider immediate reconstruction if planned',
      risks: 'Lymphedema, shoulder dysfunction, wound complications, seroma, sensory loss, infection.',
      precautions: 'Physiotherapy planning for shoulder, drain management education, and counselling about reconstruction and possible adjuvant therapy.',
      recovery: 'Hospital stay variable; drain care for 1–2 weeks; early shoulder physiotherapy to prevent stiffness; outpatient follow-up for pathology and adjuvant planning.',
      learning: '• Axillary anatomy and lymphedema risk\n• Indications for mastectomy vs breast-conserving surgery\n• Post-op rehabilitation and lymphedema prevention strategies'
    }
  };
  ...
  async function handleLocalQuery(message: string): Promise<string | null> {
    ...
    // List all distinct surgeries across patients
    if (/\b(list all surgeries|name all the surgeries|what surgeries have all patients had|what are the surgeries taken by all patients|all surgeries)\b/i.test(m)) {
      try {
        const { data, error } = await supabase.from('surgeries').select('surgery_type');
        if (error) throw error;
        if (!data || data.length === 0) return 'No surgeries found in the system.';
        const unique = Array.from(new Set(data.map((d: any) => d.surgery_type).filter(Boolean)));
        if (unique.length === 0) return 'No surgeries recorded.';
        return `Surgeries performed across patients (${unique.length} types):\n\n• ${unique.join('\n• ')}`;
      } catch (e) {
        console.error('Error listing all surgeries:', e);
        return 'I encountered an error while listing surgeries. Please try again.';
      }
    }

    // List surgeries grouped by patient (brief)
    if (/\b(list surgeries by patient|surgeries by patient|name surgeries by patient|what surgeries did all patients have|surgeries for all patients)\b/i.test(m)) {
      try {
        const { data: patients, error: pErr } = await supabase.from('patients').select('id, full_name').order('full_name', { ascending: true });
        if (pErr) throw pErr;
        if (!patients || patients.length === 0) return 'No patients found.';

        const sections: string[] = [];
        for (const p of patients) {
          try {
            const { data: surgs } = await supabase.from('surgeries').select('surgery_type').eq('patient_id', p.id).order('surgery_date', { ascending: false });
            if (!surgs || surgs.length === 0) {
              sections.push(`${p.full_name} — No surgeries recorded`);
            } else {
              const types = Array.from(new Set(surgs.map((s: any) => s.surgery_type).filter(Boolean)));
              sections.push(`${p.full_name} — ${types.join('; ')}`);
            }
          } catch (e) {
            console.error('Error fetching surgeries for patient in bulk list:', e);
            sections.push(`${p.full_name} — (error fetching surgeries)`);
          }
        }

        return `Surgeries by patient:\n\n${sections.join('\n')}`;
      } catch (e) {
        console.error('Error grouping surgeries by patient:', e);
        return 'I encountered an error while compiling surgeries by patient. Please try again.';
      }
    }

    // List all patients (brief overview)
    if (/\b(list all patients|show all patients|name all patients|list patients|show patients)\b/i.test(m)) {
      try {
        const { data, error } = await supabase.from('patients').select('full_name, department, status, admission_date').order('full_name', { ascending: true }).limit(200);
        if (error) throw error;
        if (!data || data.length === 0) return 'No patients found.';
        const rows = data.map((p: any) => `• ${p.full_name} — ${p.department || 'Unknown dept'} — ${p.status || 'Status unknown'}${p.admission_date ? ` — admitted ${new Date(p.admission_date).toLocaleDateString()}` : ''}`);
        return `Patients (${data.length}):\n\n${rows.join('\n')}`;
      } catch (e) {
        console.error('Error listing patients:', e);
        return 'I encountered an error while listing patients. Please try again.';
      }
    }
    ...
    // Flexible surgery-by-name queries: "tell me about surgery taken by Kavitha", "what surgery did Kavitha have?"
    try {
      let surgeryNameMatch = null as RegExpMatchArray | null;
      surgeryNameMatch = message.match(/(?:tell me about|what|which).{0,30}?(?:surgery|surgeries|operation|procedure).{0,30}?\bby\s+([a-z\s]+)/i)
        || message.match(/surgery taken by\s+([a-z\s]+)/i)
        || message.match(/surgery performed on\s+([a-z\s]+)/i)
        || message.match(/what (?:surgery|surgeries) did\s+([a-z\s]+)\s+have/i)
        || message.match(/([a-z\s]+)\s+(?:had|has) (?:a )?(?:surgery|operation)/i);

      if (surgeryNameMatch) {
        const rawName = surgeryNameMatch[1].trim();
        // reuse patient lookup logic: search patients by name
        const { data: patients, error: pErr } = await supabase
          .from('patients')
          .select('*')
          ...
          .limit(10);
        if (pErr) throw pErr;
        if (!patients || patients.length === 0) return `No patients found matching "${rawName}".`;

        if (patients.length > 1) {
          const rows = patients.map((p: any) => `• ${p.full_name} — ${p.department || 'Unknown dept'}`).join('\n');
          return `Multiple patients found matching "${rawName}":\n${rows}\n\nPlease repeat with the full name or select the patient in the UI for detailed info.`;
        }

        const patient = patients[0];
        const { data: surgs, error: sErr } = await supabase.from('surgeries').select('*').eq('patient_id', patient.id).order('surgery_date', { ascending: false });
        if (sErr) throw sErr;
        if (!surgs || surgs.length === 0) return `No surgeries found for ${patient.full_name}.`;
        let resp = `Surgeries for ${patient.full_name}:\n`;
        for (const s of surgs) {
      ...
      console.error('Error in surgery-by-name handler:', e);
    }

    // Educate about surgeries for all patients (e.g. "educate me about all surgeries", "teach me about surgeries for all patients")
    if (/\b(educate me about all surgeries|teach me about all surgeries|educate about surgeries for all patients|teach me about surgeries for all patients|educate me about surgeries|teach me about surgeries)\b/i.test(message)) {
      try {
        const { data: patients, error: pErr } = await supabase.from('patients').select('id, full_name').limit(50);
        if (pErr) throw pErr;
        if (!patients || patients.length === 0) return 'No patients found.';

        // For each patient, fetch their surgeries
        const patientSurgeries = await Promise.all(patients.map(async (p: any) => {
          const { data: surgs } = await supabase
            .from('surgeries')
            .select('*')
            .eq('patient_id', p.id)
            .order('surgery_date', { ascending: false });
          return { patient: p, surgeries: surgs || [] };
        }));

        // Build educational content per surgery for interns
        const sections: string[] = [];
        let totalCount = 0;
        for (const ps of patientSurgeries) {
          if (!ps.surgeries || ps.surgeries.length === 0) continue;
          for (const s of ps.surgeries) {
            totalCount += 1;
            const patientName = ps.patient.full_name;
            sections.push(`Patient: ${patientName}`);
            sections.push(`Surgery: ${s.surgery_type}`);
            sections.push(`Date: ${s.surgery_date ? new Date(s.surgery_date).toLocaleString() : 'date/time unknown'}`);
            sections.push(`Surgeon: ${s.surgeon_name || 'Unknown'}`);
            sections.push(`Duration: ${s.duration_minutes ? `${s.duration_minutes} minutes` : 'Not recorded'}`);

            // educational match
            const key = Object.keys(surgeryInfo).find(k => s.surgery_type.toLowerCase().includes(k) || k.includes(s.surgery_type.toLowerCase()));
            if (key) {
              const info = surgeryInfo[key];
              sections.push('Purpose: ' + (info.purpose || 'N/A'));
              sections.push('Procedure Overview: ' + info.procedure);
              sections.push('Key Process Steps: ' + info.process.split('\n').slice(0,5).join('; '));
              sections.push('Risks: ' + (info.risks || 'N/A'));
              sections.push('Precautions: ' + (info.precautions || 'N/A'));
              sections.push('Recovery Summary: ' + (info.recovery || 'N/A'));
              sections.push('Important Learning Points for Interns: ' + info.learning);
            } else {
              sections.push('Note: No structured educational template available for this procedure. Consider using "teach me about <procedure>" for a focused educational summary.');
            }

            // include surgery-specific notes if present
            if (s.notes) sections.push('Surgery Notes: ' + s.notes);

            // latest post-op note for context
            try {
              const { data: post } = await supabase.from('post_operative_notes').select('*').eq('patient_id', ps.patient.id).eq('surgery_id', s.id).order('day_number', { ascending: false }).limit(1);
              if (post && post.length > 0) {
                const n = post[0];
                sections.push(`Latest Post-Op — Day ${n.day_number || '?'}: Pain ${n.pain_level || 'N/A'} | Wound: ${n.wound_condition || 'N/A'}${n.complications ? ` | Complications: ${n.complications}` : ''}`);
              }
            } catch (e) {
              console.error('Error fetching post-op during bulk educate:', e);
            }
        ...
        if (totalCount === 0) return 'No surgeries found for any patients.';
        // Limit output length to avoid overly large responses
        const maxSections = 1000;
        const output = sections.join('\n');
        return `Educational summary for surgeries (${totalCount} found):\n\n${output.slice(0, maxSections)}${output.length > maxSections ? '\n\nOutput truncated. Request a specific patient or procedure for the full details.' : ''}`;
      } catch (e) {
        console.error('Error generating bulk surgeries education:', e);
        return 'I encountered an error while preparing the educational summaries. Please try again.';
      }
    }
